...
function StockChart({ id }) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      const matchingPatterns = patterns.filter(pattern => pattern.date === label);
      console.log(payload)
      return (
        <div className="custom-tooltip">
          ...
          matchingPatterns.map(pattern => {
            const sentimentClass = pattern.sentiment === 'Bullish' ? 'bullish desc' : 'bearish desc';
            return (
              <p className={sentimentClass}>{`${pattern.pattern_name}/${pattern.timeframe} - ${pattern.sentiment}`}</p>
            );
          ...
          <Line
            ...
            dot={(props) => {
              ...
              if (patterns.some(pattern => pattern.date === payload.date)) {
                const pattern = patterns.find(pattern => pattern.date === payload.date);
                return (
                  <circle 
                    cx={cx} 
                    cy={cy} 
